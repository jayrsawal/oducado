...
export default function AdminRosterEditor({ pollId }) {
  ...
  async function addTable(name) {
    ...
    setSaving(true)
    setError(null)
    const { error: insertError } = await supabase.from('poll_tables').insert({
      poll_id: pollId,
      name: trimmed,
  ...
  async function deleteTable(tableId) {
    ...
    const { error: deleteError } = await supabase.from('poll_tables').delete().eq('id', tableId)
  ...
  async function addGuest(name, tableId) {
    ...
    const { error: insertError } = await supabase.from('poll_roster').insert({
      poll_id: pollId,
  ...
  async function deleteGuest(id) {
    ...
    const { error: deleteError } = await supabase.from('poll_roster').delete().eq('id', id)
    if (deleteError) setError(deleteError.message)
    else await load()
  }
  ...
  return (
    <div className="admin-roster">
      <p className="poll-hint">
        Organize guests by table. They&apos;ll pick their name from the grouped list when voting.
      </p>
      ...
      {tables.length === 0 && entries.length === 0 ? (
        <p className="poll-empty">Add tables and guests before opening the poll.</p>
      ) : (
